...
exports.reportCSV = reportCSV;
exports.reportJUnit = reportJUnit;
...
function reportTerminal(report) {
    ...
    if (report.suppressedCount && report.suppressedCount > 0) {
        console.log(chalk_1.default.gray(`    ${report.suppressedCount} finding(s) suppressed by baseline`));
        if (report.newFindings !== undefined) {
            console.log(chalk_1.default.gray(`    ${report.newFindings} new finding(s)`));
        }
        console.log('');
    }
    console.log(chalk_1.default.gray('  ══════════════════════════════════════════'));
...
function reportSARIF(report) {
    ...
    // Build results
    const sarifResults = report.results.flatMap(result => result.vulnerabilities.map(vuln => ({
        ruleId: vuln.rule,
...
function reportMarkdown(report) {
    ...
    for (const result of report.results) {
        for (const vuln of result.vulnerabilities) {
            ...
            md += `\n`;
            if (vuln.cwe)
                md += `- **CWE:** ${escapeText(vuln.cwe)}\n`;
            if (vuln.owasp)
                md += `- **OWASP:** ${escapeText(vuln.owasp)}\n`;
...
function reportJUnit(report) {
    ...
    for (const result of report.results) {
        for (const vuln of result.vulnerabilities) {
            ...
            if (severity === 'critical' || severity === 'high') {
                failures++;
                testCases.push(`    <testcase name="${escapeXml(tcName)}" classname="${escapeXml(className)}" time="0.0">\n` +
    ...
    if (testCases.length === 0) {
        testCases.push(`    <testcase name="appshield.scan" classname="${escapeXml(report.projectPath)}" time="${duration}" />`);
    }
    const xml = [
...
function mdSanitizeFences(text) {
